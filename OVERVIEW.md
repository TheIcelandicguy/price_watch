...
- **Version:** 0.2.0 (first public beta) · **Minimum HA:** 2024.10.0
- **License:** MIT
...
- **Region-awareness** — a shipping heuristic flags (and can hide) listings that won't ship to the user's country; a global excluded-domains blocklist drops unwanted hosts.
...
- **Resilient fetching** — curl_cffi Chrome-131 TLS impersonation, fresh-session retry for bot-walls (Amazon "Continue shopping"), a per-host politeness gap, and a global concurrency cap.
...
│   ├── coordinator_alternatives.py    # AlternativesMixin — discovery + result filtering
...
├── scripts/                           # ad-hoc retailer probe/diagnostic scripts (NOT deploy)
└── tests/                             # pytest suite
```
...
secondary failure is logged and the tick still succeeds.
...
and a 3 s per-host politeness gap.
...
Broadcast services resolve targets by `entry_id`, `device_id`, or all products.
...
3. **Change detection** — the page's text is SHA-256 hashed; if it equals the
   ...
   result is reused (FX is still recomputed for the primary listing).
...
pytest tests/          # test_extractor, test_parsers, test_fx, test_presets_*, test_alternatives_filter, test_diagnostics
...
CI (`.github/workflows`) runs hassfest, HACS validation, pytest (3.12 + 3.13)
and ruff on every push. `pytest.ini` and `requirements_test.txt` configure the
test env; `.ruff_cache` is local.

**Deployment (developer workflow):** the working repo lives on `E:\price_watch`
and is deployed to the HA config's `Z:\custom_components\price_watch`. There is
no deploy script committed in the repo — the deploy is a manual mirror
(robocopy `/E`); on Windows an occasional file lock / EPERM on the running
component is worked around by retrying. After copying, restart HA. The panel
bundle must be built (`npm run build`) before the sidebar registers; otherwise
`panel.py` logs a warning and skips registration while the rest of the
...
specially. **Hard fact to preserve: Komplett.no does NOT ship to Iceland**, so
it should never be recommended as an Icelandic retailer / alternative. The
...
- **Secondary listings are backend-complete but not yet fully surfaced** in the
  current panel (Phase 4 work); `price_local` and FX are product-level today.
